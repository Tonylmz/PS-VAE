...
def config_from_atoms_pkl(
    ...
    atomic_numbers = np.load(file=r"/home/u2023000178/lmz/mace/ESTAG-master/data2.npy")
    forces = atoms[1].numpy()
    positions = atoms[0].numpy()
    cell = 50 * np.identity(3)
...
def config_from_atoms_list_pkl(
    atoms_list: List[tuple],
    energy_key="energy",
    forces_key="forces",
    stress_key="stress",
    virials_key="virials",
...
def load_from_pkl(
    ...
    for path in os.listdir(file_path):
        atoms_list.append(torch.load(os.path.join(file_path, path)))

    atomic_energies_dict = {}
    

    configs = config_from_atoms_list_pkl(
        atoms_list,
        config_type_weights=config_type_weights,
        energy_key=energy_key,
        forces_key=forces_key,
