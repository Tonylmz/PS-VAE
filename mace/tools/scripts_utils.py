...
import sys
sys.path.append("mace")
import data
from data import AtomicData
from tools import AtomicNumberTable, evaluate, torch_geometric
...
def get_dataset_from_pkl(
    ...
    """Load training and test dataset from xyz file"""
    atomic_energies_dict, all_train_configs = data.load_from_pkl(
        file_path=train_path,
        config_type_weights=config_type_weights,
        energy_key=energy_key,
        forces_key=forces_key,
    ...
    if valid_path is not None:
        _, valid_configs = data.load_from_pkl(
            file_path=valid_path,
            config_type_weights=config_type_weights,
            energy_key=energy_key,
            forces_key=forces_key,
    ...
    test_configs = []
    if test_path is not None:
        _, all_test_configs = data.load_from_pkl(
            file_path=test_path,
            config_type_weights=config_type_weights,
            energy_key=energy_key,
            forces_key=forces_key,
    ...
    for name, subset in all_collections:
        ...
        if table_type == "TotalRMSE":
            table.add_row(
                [
                    name,
                    f"{metrics['rmse_e'] * 1000:.3f}",
                    f"{metrics['rmse_f'] * 1000:.3f}",
                    f"{metrics['rel_rmse_f']:.3f}",
                ]
            )
        elif table_type == "PerAtomRMSE":
            table.add_row(
            ...
            table.add_row(
                [
                    ...
                    f"{metrics['rmse_f'] * 1000:.3f}",
                    f"{metrics['rel_rmse_f']:.3f}",
                    f"{metrics['rmse_virials'] * 1000:.3f}",
                ]
            )
        elif table_type == "TotalMAE":
            table.add_row(
                [
                    ...
                    f"{metrics['mae_f'] * 1000:.3f}",
                    f"{metrics['rel_mae_f']:.3f}",
                ]
            )
        elif table_type == "PerAtomMAE":
            table.add_row(
            ...
            table.add_row(
                [
                    name,
                    f"{metrics['rmse_mu_per_atom'] * 1000:.3f}",
                    f"{metrics['rel_rmse_mu']:.3f}",
                ]
            )
        elif table_type == "DipoleMAE":
            table.add_row(
                [
                    name,
                    f"{metrics['mae_mu_per_atom'] * 1000:.3f}",
                    f"{metrics['rel_mae_mu']:.3f}",
                ]
            )
        elif table_type == "EnergyDipoleRMSE":
            table.add_row(
