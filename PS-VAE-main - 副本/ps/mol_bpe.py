...
class MolInSubgraph:
    def __init__(self, mol, kekulize=False):
        ...
        for atom in mol.GetAtoms():
            self.inversed_index[atom.GetIdx()] = set()
        ssr = Chem.GetSymmSSSR(mol)
        for ring in ssr:
            # self.subgraphs[self.upid_cnt] = { aid: mol.GetAtomWithIdx(aid).GetSymbol() for aid in ring }
            self.subgraphs[self.upid_cnt] = {}
            for aid in ring:
                atom = mol.GetAtomWithIdx(aid)
                idx, symbol = atom.GetIdx(), atom.GetSymbol()
                self.inversed_index[idx].add(self.upid_cnt)
                self.subgraphs[self.upid_cnt][idx] = symbol
            submol = get_submol(mol, list(ring), kekulize=kekulize)
            self.subgraphs_smis[self.upid_cnt] = mol2smi(submol)
            self.upid_cnt += 1

        for atom in mol.GetAtoms():
            idx, symbol = atom.GetIdx(), atom.GetSymbol()
            # for nei in atom.GetNeighbors():
            #     nei_idx, nei_symbol = nei.GetIdx(), nei.GetSymbol()
            #     edge = mol.GetBondBetweenAtoms(idx, nei_idx)
                # if (not atom.IsInRing() and not nei.IsInRing() and nei_idx > idx) or (atom.IsInRing() and not edge.IsInRing()):
                #     self.subgraphs[self.upid_cnt] = { idx: symbol, nei_idx: nei_symbol }
                #     submol = get_submol(mol, [idx, nei_idx], kekulize=kekulize)
                #     self.subgraphs_smis[self.upid_cnt] = mol2smi(submol)
                #     self.inversed_index[idx].add(self.upid_cnt)
                #     self.inversed_index[nei_idx].add(self.upid_cnt)
                #     self.upid_cnt += 1  
            if not atom.IsInRing():
                for nei in atom.GetNeighbors():
                    nei_idx, nei_symbol = nei.GetIdx(), nei.GetSymbol()
                    if not nei.IsInRing() and nei_idx > idx:
                        self.subgraphs[self.upid_cnt] = { idx: symbol, nei_idx: nei_symbol }
                        submol = get_submol(mol, [idx, nei_idx], kekulize=kekulize)
                        self.subgraphs_smis[self.upid_cnt] = mol2smi(submol)
                        self.inversed_index[idx].add(self.upid_cnt)
        ...
        # print(self.inversed_index)
        init_smi = {}
        for key in self.subgraphs.keys():
            subgraph = self.subgraphs[key]
            submol = get_submol(mol, list(subgraph.keys()), kekulize=kekulize)
            smi = mol2smi(submol)
            if smi in init_smi:
                init_smi[smi] += 1
            else:
    ...
    def get_init_smi(self):
        return self.init_smi
    
    def get_nei_subgraphs(self):
        
        nei_subgraphs, merge_pids = [], []
        for key1 in self.subgraphs:
            subgraph1 = self.subgraphs[key1]
            key_set1 = set(subgraph1.keys())
            for key2 in self.subgraphs:
                subgraph2 = self.subgraphs[key2]
                key_set2 = set(subgraph2.keys())
                if key1 != key2 and len(key_set1 & key_set2) != 0 and {key1, key2} not in merge_pids:
                    new_subgraph = copy(subgraph1)
                    new_subgraph.update(subgraph2)
                    nei_subgraphs.append(new_subgraph)
                    merge_pids.append({key1, key2})
        # for key in self.subgraphs:
        #     subgraph = self.subgraphs[key]
        #     local_nei_pid = []
        #     for aid in subgraph:
    ...
    def get_nei_smis(self):
        if self.dirty:
            ...
            # print("merge_pids", merge_pids)
            # print("nei_subgraphs", nei_subgraphs)
    ...
    def merge(self, smi):
        if self.dirty:
            self.get_nei_smis()
        # print("smi", smi)
        # print("smi2pids", self.smi2pids)
        # print("subgraphs", self.subgraphs)
        # print("inversed_index", self.inversed_index)
        if smi in self.smi2pids:
            ...
            for pid1, pid2 in merge_pids:
                # self.subgraphs[self.upid_cnt] = self.subgraphs[pid1] | self.subgraphs[pid2]
                # self.subgraphs_smis[self.upid_cnt] = smi
                # for aid in self.subgraphs[pid1].keys():
                #     self.inversed_index[aid].discard(pid1)
                #     self.inversed_index[aid].discard(pid2)
                #     self.inversed_index[aid].add(self.upid_cnt)
                # for aid in self.subgraphs[pid2].keys():
                #     self.inversed_index[aid].discard(pid1)
                #     self.inversed_index[aid].discard(pid2)
                #     self.inversed_index[aid].add(self.upid_cnt)
                # self.upid_cnt += 1
                if pid1 in self.subgraphs and pid2 in self.subgraphs: # possibly del by former
                    ...
                    # print("subgraphs_smis", self.subgraphs_smis)
                    self.subgraphs_smis[self.upid_cnt] = smi
                    ...
                    self.upid_cnt += 1
            # for pid1, pid2 in merge_pids:
            #     if pid1 in self.subgraphs:
            #         del self.subgraphs[pid1]
            #         del self.subgraphs_smis[pid1]
            #     if pid2 in self.subgraphs:
            #         del self.subgraphs[pid2]
            #         del self.subgraphs_smis[pid2]
        # print(111111)
        # print("smi2pids", self.smi2pids)
    ...
    # def del_subgraphs(self, subgraphs):
    #     for key in subgraphs.keys():
    #         del self.subgraphs[key]
    #         del self.subgraphs_smis[key]
...
def graph_bpe(fname, vocab_len, vocab_path, cpus, kekulize):
    ...
    for smi in tqdm(smis):
        ...
        try:
            mol = MolInSubgraph(smi2mol(smi, kekulize), kekulize)
            # print(mol.get_nei_smis())
            # print(mol.get_nei_subgraphs())
            mols.append(mol)
    ...
    # selected_smis, details = [], {}   # details: <smi: [atom cnt, frequency]
    ...
    # # selected_smis_init, details_init = [], {}
    # for mol in mols:
    #     init_smi = mol.get_init_smi()
    #     # print(init_smi)
    #     for smi in init_smi.keys():
    #         if smi not in details:
    #             details[smi] = [cnt_atom(smi), init_smi[smi]]
    #             selected_smis.append(smi)
    #         else:
    #             details[smi][1] += init_smi[smi]
